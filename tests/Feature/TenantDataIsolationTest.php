...
class TenantDataIsolationTest extends TestCase
{
    ...
    public function test_login_requires_workspace(): void
    {
    ...
    public function test_login_rejects_email_for_wrong_workspace(): void
    {
        $this->seedTenantUser($this->tenantA, 'owner@a.test', 'secret123');

        $this->postJson('/api/tenant/login', [
            'workspace' => $this->tenantB->slug,
            'email' => 'owner@a.test',
            'password' => 'secret123',
        ])->assertStatus(422)
            ->assertJsonPath('success', false);
    ...
    public function test_login_succeeds_only_for_matching_workspace_and_directory(): void
    {
        $this->seedTenantUser($this->tenantA, 'owner@a.test', 'secret123');

        $this->postJson('/api/tenant/login', [
            'workspace' => $this->tenantA->slug,
            'email' => 'owner@a.test',
            'password' => 'secret123',
    ...
    public function test_token_is_bound_to_tenant_and_cannot_access_other_workspace(): void
    {
        $this->seedTenantUser($this->tenantA, 'owner@a.test', 'secret123');
        ...
        $this->getJson('/api/tenant/customers?per_page=1', [
            ...
            ->assertJsonPath('message', 'Token is not valid for this tenant workspace');
    }
